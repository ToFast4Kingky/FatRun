...
namespace EnhancedHierarchy {
    internal static partial class Preferences {
        ...
        private static readonly Version pluginVersion = new Version(2, 4, 0);
        private static readonly DateTime pluginDate = new DateTime(2018, 10, 14);
        ...
        private static SettingsProvider RetrieveSettingsProvider() {
            var settingsProvider = new SettingsProvider("Hierarchy");

            settingsProvider.scopes = SettingsScopes.User;
            settingsProvider.guiHandler = new Action<string>((str) => OnPreferencesGUI(str));

            foreach(var cont in contents)
                settingsProvider.keywords.Add(cont.text);
            ...
            using(Enabled.GetEnabledScope()) {
                using(new GUIIndent("Misc settings")) {
                    ...
                    using(new GUIIndent("Margins")) {
                        ...
                        using(new GUIEnabled(ReflectionHelper.HierarchyArea.Supported)) {
                            LeftMargin.DoGUISlider(-50, 50);
                            Indent.DoGUISlider(0, 35);
                        }
                        if(!ReflectionHelper.HierarchyArea.Supported)
                    ...
                    using(new GUIEnabled(ReflectionHelper.IconWidthSupported))
